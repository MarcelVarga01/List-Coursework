...
list *newList(item e){
  ...
  newlist->none = sentinel;
  newlist->current = sentinel;
...
void insertAfter(list *xs, item x){
  ...
  //initialize value held by the new node
  new->x=x;
...
void insertBefore(list *xs, item x){
  ...
  //initialize value held by new node
  new->x=x;

  //establish new connecitons
